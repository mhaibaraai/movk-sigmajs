...
```text
src/module.ts                          模块入口：alias #sigma、注册组件与自动导入
src/optimize-deps.ts                   探测已安装的可选 peer，生成 vite optimizeDeps.include
...
- 模块选项为 `prefix` / `settings` / `css` / `optimizeDeps`，其中全局默认 settings 写入 `runtimeConfig.public.sigma.settings`
