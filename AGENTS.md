...
- **`patches/sigma@*.patch` 的退出方式是删除，不是移植。** 上游已合入这两处缺陷的修复——`79752e39`（隐藏元素别名纹理第 0 行，即 [PR #1550](https://github.com/jacomyal/sigma.js/pull/1550)）与 `5ecdf690`（隐藏边导致后续边属性行错位，维护者自行修的 issue #1549 第二部分）。但**上游走的是 `HIDDEN_ITEM_INDEX = -1` 哨兵加顶点着色器裁剪，本仓库 patch 走的是「保留第 0 行」（`nextIndex = 1`），正是上游 PR 里被显式否掉的备选方案**。所以 sigma 发布含修复的版本后，直接删掉 `patches/`、`pnpm-workspace.yaml` 的 `patchedDependencies`、README 与 `use-sigma-filter` / `use-sigma-state` 两页的接入说明，不要把上游的哨兵实现搬进 patch。npm 上最新的 `4.0.0-beta.1`（2026-08-12）早于两个修复，上游也没有 canary 发布通道，所以在此之前 patch 仍是必需的
