...
::field-group
  ...
  组件名前缀。组件文件本身不带前缀，由此选项统一加上。默认 `'Sigma'`，即 `Graph.vue` 注册为 `<SigmaGraph>`。
  :::

  :::field{name="settings" type="Partial<Settings>"}
  全局默认的 sigma 渲染配置，与组件级 `settings` 深度合并后整体透传。不做键白名单，sigma 新增的配置项无需本模块升级即可使用。
  :::

  :::field{name="css" type="boolean"}
  注入内置控件与覆盖层的样式表。默认 `true`；设为 `false` 后控件的结构与行为不受影响，只是不带样式。
  :::
::
